...
vk::DescriptorSets CreateWrappedDescriptorSets(vk::DescriptorPool& pool,
    ...
    // from the one live render device instead -- frame generation is built and torn down inside
    // that device's lifetime, and there is never a second one. Keeping the device out of the
    // signature is deliberate: it leaves all six call sites in the pass files identical to Eden's.
    // PCSX2 reached this through GSDeviceVK::GetInstance(); RPCS3's equivalent is the global
    // below, which is null outside a live renderer for the same reason.
    const ::vk::render_device* const render_dev = ::vk::g_render_device;
    if (!render_dev) {
        return {};
    }
    const VkDevice device = static_cast<VkDevice>(*render_dev);
    ...
    if (res != VK_SUCCESS) {
        framegen_log.error("LSFG: vkAllocateDescriptorSets() failed: %d", static_cast<int>(res));
        return {};
    }
