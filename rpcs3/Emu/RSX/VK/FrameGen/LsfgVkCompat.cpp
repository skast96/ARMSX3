...
namespace Vulkan
{
	namespace vk
	{
		...
		ImageView LogicalDevice::CreateImageView(const VkImageViewCreateInfo& ci) const
		{
			...
			if (res != VK_SUCCESS)
			{
				framegen_log.error("LSFG: vkCreateImageView() failed: %d", static_cast<int>(res));
				return {};
			}
		...
		Sampler LogicalDevice::CreateSampler(const VkSamplerCreateInfo& ci) const
		{
			...
			if (res != VK_SUCCESS)
			{
				framegen_log.error("LSFG: vkCreateSampler() failed: %d", static_cast<int>(res));
				return {};
			}
		...
		ShaderModule LogicalDevice::CreateShaderModule(const VkShaderModuleCreateInfo& ci) const
		{
			...
			if (res != VK_SUCCESS)
			{
				framegen_log.error("LSFG: vkCreateShaderModule() failed: %d", static_cast<int>(res));
				return {};
			}
		...
		DescriptorSetLayout LogicalDevice::CreateDescriptorSetLayout(const VkDescriptorSetLayoutCreateInfo& ci) const
		{
			...
			if (res != VK_SUCCESS)
			{
				framegen_log.error("LSFG: vkCreateDescriptorSetLayout() failed: %d", static_cast<int>(res));
				return {};
			}
		...
		PipelineLayout LogicalDevice::CreatePipelineLayout(const VkPipelineLayoutCreateInfo& ci) const
		{
			...
			if (res != VK_SUCCESS)
			{
				framegen_log.error("LSFG: vkCreatePipelineLayout() failed: %d", static_cast<int>(res));
				return {};
			}
		...
		Pipeline LogicalDevice::CreateComputePipeline(const VkComputePipelineCreateInfo& ci) const
		{
			...
			if (res != VK_SUCCESS)
			{
				framegen_log.error("LSFG: vkCreateComputePipelines() failed: %d", static_cast<int>(res));
				return {};
			}
		...
		DescriptorPool LogicalDevice::CreateDescriptorPool(const VkDescriptorPoolCreateInfo& ci) const
		{
			...
			if (res != VK_SUCCESS)
			{
				framegen_log.error("LSFG: vkCreateDescriptorPool() failed: %d", static_cast<int>(res));
				return {};
			}
	...
	Device::Device(const ::vk::render_device* dev)
		: m_device(dev)
	{
		if (!dev)
			return;

		m_logical = vk::LogicalDevice(static_cast<VkDevice>(*dev));
		...
		//
		// RPCS3 probes both in device.cpp and, since the change that accompanied this port,
		// ENABLES them at vkCreateDevice under the same condition -- so support and enablement
	...
	vk::Image MemoryAllocator::CreateImage(const VkImageCreateInfo& ci) const
	{
		...
		if (res != VK_SUCCESS)
		{
			framegen_log.error("LSFG: vmaCreateImage() failed: %d", static_cast<int>(res));
			return {};
		}
	...
	vk::Buffer MemoryAllocator::CreateBuffer(const VkBufferCreateInfo& ci, MemoryUsage usage) const
	{
		...
		if (res != VK_SUCCESS)
		{
			framegen_log.error("LSFG: vmaCreateBuffer() failed: %d", static_cast<int>(res));
			return {};
		}
