...
private fun ControllerOptions(
    ...
    GlassPanel(modifier) {
        Column(verticalArrangement = Arrangement.spacedBy(7.dp)) {
            ...
            // Seven ports, matching the Pad tab and PadRouter. Two weighted chips
            // fitted the row; seven would each be ~14% wide and clip their label, so
            // this scrolls instead and the labels shorten to P1..P7.
            Row(
                horizontalArrangement = Arrangement.spacedBy(7.dp),
                modifier = Modifier.horizontalScroll(rememberScrollState()),
            ) {
                for (p in 0 until com.armsx2.input.PadRouter.MAX_PADS) {
                    ChoiceRow(
                        if (compact) "P${p + 1}" else str("pad.player${p + 1}"),
                        state.player == p,
                        { viewModel.setPlayer(p) },
                        Modifier.controllerFocusable(
                            "controls.player${p + 1}",
                            RoundedCornerShape(14.dp),
                            onConfirm = { viewModel.setPlayer(p) },
                        ),
                    )
                }
            }
            ...
            // No multitap row: the PS3 has seven ports and no accessory to enable.
            SettingSwitchRow(
